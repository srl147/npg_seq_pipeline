...
sub _populate_cluster_counts {
  ...
  my $interop_file = $self->runfolder_path().q{/InterOp/TileMetricsOut.bin};
  if ( ! -e $interop_file ) {
    $self->logwarn(qq{Couldn't find interop file $interop_file, looking in $PF_CYCLE sub-directory});
    # look for one in the PF_CYCLE sub-directory
    $interop_file = $self->runfolder_path().qq{/InterOp/$PF_CYCLE/TileMetricsOut.bin};
    if ( ! -e $interop_file ) {
      $self->logcroak(qq{Couldn't find interop file $interop_file});
    }
  }
  my $interop = $self->parsing_interop($interop_file);
